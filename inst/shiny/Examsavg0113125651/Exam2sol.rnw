...
\begin{enumerate}[(a)]
\item  \Sexpr{mean(x)}  
{\bf T }\item \Sexpr{mean(x)+1}  
{\bf F }\item does not exist 
{\bf F }\item \Sexpr{median(x)}  
{\bf F }\end{enumerate}
...
<<results=hide, echo=false>>=
res<-round(pbinom(succ, n, 0.5), 4)
@
Number of trials: n=\Sexpr{n}, Number of successes=k=\Sexpr{succ},  probability of success: p=0.5. Using binomial distribution: $P(X=k|n,p)=$\Sexpr{res}. }\vspace{\baselineskip} \vspace{\baselineskip} \vspace{\baselineskip}\end{itemize}
\newpage
\end{document}