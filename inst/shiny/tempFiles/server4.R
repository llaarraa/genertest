shinyServer(function(input, output) {
  ...
  #output$topics.points=vector("list", length(my.topics))
  
  
#  for(i in 1:length(my.topics))
    
    
 # output$topics.points[[i]] <- renderUI({
  #  # If missing input, return to avoid error later in function
   
#    numericInput("tot.points", "Total number of points to include in each test:", 50, min=0)
    
    
 #   #matrixInput(topicsPoints, topics, matrix(rep(0, length(topics)), nrows=length(topics)))
#  })
